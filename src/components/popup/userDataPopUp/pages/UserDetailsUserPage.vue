...
async function saveUserChanges() {
  ...
  const newData = {id:userDataStore.selectedUser.id as string,email:userEmail.value,name:userName.value,typeId:userType.value} as User
  const updatedUser = await UserService.updateUser(newData)
  userDataStore.saveInProgress = false;
  if (updatedUser) {
    userDataStore.selectedUser.email = updatedUser.email
    userDataStore.selectedUser.name = updatedUser.name
    userDataStore.selectedUser.typeId = updatedUser.typeId
  }
}
...
<template>
  <form @submit.prevent class="flex flex-col">
      ...
      >
        <div class="flex flex-col grow items-stretch max-md:mt-10">
          <div class="flex flex-row justify-center items-center">
            ...
            <img src="@/assets/images/edit_button.png" class="w-[25px] h-[25px] absolute right-4 z-10"
                 v-if="!editInProgress" @click="startEdit()">
          </div>
    ...
    <div
      ...
      v-if="editInProgress"
    ></div>
    <div class="flex flex-row justify-around items-center" v-if="editInProgress">
      <button type="button"
